...
<div class="row">
    <div class="col-md-6">
        @foreach($flight_data['flight_destination'] as $id => $flight)
            <h2><i class="fa fa-arrow-right"></i> {{ trans('mamorunl-ota::flight.display.flight_information') }} {{ $flight['airline_code'] }} {{ $flight['flight_number'] }}: {{ $flight['date_departure'] }}</h2>
            <div class="row">
                <div class="col-md-6">
    ...
    <div class="col-md-6">
        @foreach($flight_data['flight_home'] as $id => $flight)
            <h2><i class="fa fa-arrow-left"></i>{{ trans('mamorunl-ota::flight.display.flight_information') }} {{ $flight['airline_code'] }} {{ $flight['flight_number'] }}: {{ $flight['date_departure'] }}</h2>
            <div class="row">
                <div class="col-md-6">
                    <h3>{{ trans('soap.availability.flight_info') }}</h3>
                    <table class="table">
                ...
                <div class="col-md-6">
            <span class="ajax-price-list" data-direction="b" data-flight_number="{{ $flight['flight_number'] }}" data-airport_from="{{ $flight['airport_from'] }}" data-airport_to="{{ $flight['airport_to'] }}"
                  data-departure_dt="{{ $flight['date_departure'] }}" data-row_data="{{ serialize($flight['seats_free']) }}">&&<br/>Loading prices...</span>
                </div>
            </div>
        @endforeach
...
<script>
    $(document).ready(function() {
        $('#btn-submit').click(function(e) {
            if($('input[name=row_letter_b]:checked').val() === undefined) {
                e.preventDefault();
                alert('You haven\'t selected a return flight');
            }

            if($('input[name=row_letter_t]:checked').val() === undefined) {
                e.preventDefault();
                alert('You haven\'t selected a flight to your destination');
            }
        });
