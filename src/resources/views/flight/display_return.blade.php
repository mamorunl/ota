@foreach($flight_data['flight_destination'] as $id => $flight)
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <span class="ajax-price-list" data-flight_number="{{ $flight['flight_number'] }}" data-airport_from="{{ $flight['airport_from'] }}" data-airport_to="{{ $flight['airport_to'] }}"
            ...
            $.post('{{ route('ota.flight.load_price') }}', {
                ...
                for(var key in data) {
                    if(data.hasOwnProperty(key)) {
                        html += '<div>' + key + ': ' + data[key] +
                                '&nbsp; <a href="/link/' + key + '" class="btn btn-primary">Book now</a>'
                                '</div>';
                    }
                }
